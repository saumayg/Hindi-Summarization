...
class DataTrainingArguments:
    ...
    id_column: Optional[str] = field(
        default=None,
        metadata={"help": "The name of the column in the datasets containing the id (for summarization)."},
    )
    train_file: Optional[str] = field(
        default=None, metadata={"help": "The input training data file (a jsonlines or csv file)."}
    )
    train_crawl_file: Optional[str] = field(
        default=None, metadata={"help": "The input training data crawl file (a jsonlines or csv file)."}
    )
    validation_file: Optional[str] = field(
        default=None, metadata={"help": "The input validation data file (a jsonlines or csv file)."}
    )
    val_crawl_file: Optional[str] = field(
        default=None, metadata={"help": "The input validation data crawl file (a jsonlines or csv file)."}
    )
    test_file: Optional[str] = field(
        default=None, metadata={"help": "The input test data file (a jsonlines or csv file)."}
    )
    test_crawl_file: Optional[str] = field(
        default=None, metadata={"help": "The input test data crawl file (a jsonlines or csv file)."}
    )
    ...
    num_article: Optional[int] = field(
        default=3,
        metadata={"help": "Number of articles to append at the end of the original article"}
    )
    ...
    def __post_init__(self):
        if (
            (self.train_file is None or self.train_crawl_file is None)
            and (self.validation_file is None or self.val_crawl_file is None)
            and (self.test_file is None or self.test_crawl_file is None)
        ):
            ...
            if self.train_crawl_file is not None:
                extension = self.train_crawl_file.split(".")[-1]
                assert extension in ["jsonl"], "`train_file` should be a jsonl file."
            if self.val_crawl_file is not None:
                extension = self.val_crawl_file.split(".")[-1]
                assert extension in ["jsonl"], "`validation_file` should be a jsonl file."
            if self.test_crawl_file is not None:
                extension = self.test_crawl_file.split(".")[-1]
...
def main():
    ...
    train_crawl_data = None
    val_crawl_data = None
    test_crawl_data = None
    if data_args.train_crawl_file is not None:
        train_crawl_data = dict()
        print("Loading train crawl data")
        with open(data_args.train_crawl_file, 'rb') as file:
            for line in tqdm(file):
                rec = json.loads(line)
                train_crawl_data[rec['key']] = rec['data'][:data_args.num_article]
    if data_args.val_crawl_file is not None:
        val_crawl_data = dict()
        print("Loading val crawl data")
        with open(data_args.val_crawl_file, 'rb') as file:
            for line in tqdm(file):
                rec = json.loads(line)
                val_crawl_data[rec['key']] = rec['data'][:data_args.num_article]
    if data_args.test_crawl_file is not None:
        test_crawl_data = dict()
        print("Loading test crawl data")
        with open(data_args.test_crawl_file, 'rb') as file:
            for line in tqdm(file):
                rec = json.loads(line)
                test_crawl_data[rec['key']] = rec['data'][:data_args.num_article]
    ...
    id_column = data_args.id_column
    if id_column not in column_names:
        raise ValueError(
            f"--id_column' value '{data_args.id_column}' needs to be one of: {', '.join(column_names)}"
        )
    # if data_args.do_ner:
    ...
    def preprocess_function_train(examples):
        ...
        for i in range(len(examples[text_column])):
            if examples[text_column][i] is not None and examples[summary_column][i] is not None:
                ...
                # else:
                inp_str = f"{prefix}{examples[text_column][i]}"
                for article in train_crawl_data[examples[id_column][i]]:
                    inp_str += article['text'].replace('\u200d', '')
    ...
    def preprocess_function_val(examples):
        ...
        for i in range(len(examples[text_column])):
            if examples[text_column][i] is not None and examples[summary_column][i] is not None:
                ...
                # else:
                inp_str = f"{prefix}{examples[text_column][i]}"
                for article in val_crawl_data[examples[id_column][i]]:
                    inp_str += article['text']
    ...
    def preprocess_function_test(examples):
        ...
        for i in range(len(examples[text_column])):
            if examples[text_column][i] is not None and examples[summary_column][i] is not None:
                ...
                # else:
                inp_str = f"{prefix}{examples[text_column][i]}"
                for article in test_crawl_data[examples[id_column][i]]:
                    inp_str += article['text']
    ...
    if training_args.do_train:
        ...
        if train_crawl_data is None:
            raise RuntimeError(
                "Train crawl data should not be none"
            )
    ...
    del train_crawl_data

    if training_args.do_eval:
        # max_target_length = data_args.max_target_length
        eval_dataset = raw_datasets["validation"]
        if val_crawl_data is None:
            raise RuntimeError(
                "Val crawl data should not be none"
            )
    ...
    if training_args.do_predict:
        ...
        if test_crawl_data is None:
            raise RuntimeError(
                "Test crawl data should not be none"
            )
